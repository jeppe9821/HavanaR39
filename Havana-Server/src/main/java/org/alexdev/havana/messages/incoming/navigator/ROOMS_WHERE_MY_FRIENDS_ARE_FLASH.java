...
public class ROOMS_WHERE_MY_FRIENDS_ARE_FLASH implements MessageEvent {
    @Override
    public void handle(Player player, NettyRequest reader) throws Exception {
        var roomList = RoomManager.getInstance().getRoomsByMode(4,player);

        RoomManager.getInstance().sortRooms(roomList);
        RoomManager.getInstance().ratingSantiyCheck(roomList);

        player.send(new ROOMS_LIST_FLASH(player, roomList, -5));
    }
}
